...
<form action="{{ route("admin.appearance.homepage.find.course.update") }}" enctype="multipart/form-data" method="POST">
    ...
    <div class="form-group">
        ...
        <textarea name="title" id="categoryTitle" class="form-control" rows="5">{!! $findCourse['title'] !!}</textarea>
    </div>
    ...
    <div class="form-group">
      <label for="courses">Sellect Product For Display</label>
         <select class="form-control select3" name="courses[]" id="courses" multiple>
            @foreach ($courses as $course)
                <option 
                @foreach ($findCourse['courses'] as $course_id)
